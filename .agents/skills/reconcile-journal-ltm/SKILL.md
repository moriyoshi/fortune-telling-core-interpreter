...
Do not require LTM to preserve transient noise such as timestamps, conversational framing, or command output that adds no durable guidance.
...
If the missing coverage would introduce new topic documents or a meaningful reclustering of topics, present that plan before writing, consistent with the `good-sleep` workflow.
...
After the canonical record is in place, also delete any substantive journal entry whose durable knowledge is already represented in `.agents/docs/LTM/` according to the canonical record's section-to-LTM mapping table. LTM is the durable form; the journal carries only entries that have not yet been consolidated, plus the canonical record itself. An entry whose durable knowledge has not been promoted to LTM, for any reason, must not be removed.

This skill is the explicit exception to the usual append-only preference for `JOURNAL.md`: when the user invokes this skill, removing both superseded consolidation-record sections and consolidated substantive entries is part of the requested cleanup.

## Validation

Before finishing, verify all of the following:

1. Every substantive journal entry that remains in the file is either not yet consolidated to LTM or is the canonical `## LTM Consolidation Record` itself.
...
4. No journal entry was removed unless its durable knowledge is already represented in `.agents/docs/LTM/` according to the canonical record.
5. Repo-authored documentation still follows the `AGENTS.md` style rules: half-width parentheses, half-width colons followed by a space, and no full-width punctuation.
